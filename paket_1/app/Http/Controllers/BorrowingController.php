...
class BorrowingController extends Controller
{
    ...
    public function create()
    {
        ...
        // If admin, also get list of users to assign borrowing to
        $users = [];
        if (Auth::user()->role === 'admin') {
            $users = User::where('role', 'peminjam')->get();
        }
        
        return view('borrowings.create', compact('equipment', 'users'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // Admin must specify borrower_id
        if (Auth::user()->role === 'admin') {
            $rules['borrower_id'] = 'required|exists:users,id';
        }
        
        $request->validate($rules);

        DB::transaction(function () use ($request) {
            $borrower_id = Auth::user()->role === 'admin' ? $request->borrower_id : Auth::id();
